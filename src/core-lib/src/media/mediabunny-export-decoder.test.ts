...
describe("ExportFrameDecoder", () => {
  ...
  it("reseeks instead of walking frame-by-frame on a large forward jump (e.g. a trim in-point change)", async () => {
    class MockOffscreenCanvas {
      width: number;
      height: number;
    ...
    // One frame every second for a full minute — a forward jump from 6s to
    // 40s (like a trim moving the in-point) would mean walking through 34
    // frames one at a time if the iterator weren't reset.
    const frames = Array.from({ length: 60 }, (_, timestamp) => ({
      canvas: new MockOffscreenCanvas(320, 180),
      timestamp,
      duration: 1,
    }));
    ...
    await decoder.initialize();
    await decoder.getFrame(6);
    canvases.mockClear();

    await decoder.getFrame(40);

    // A small forward step keeps walking the same iterator (no reseek);
    // a jump past the threshold must reseek near the new timestamp instead
    // of silently decoding every frame in between inside one call.
    expect(canvases).toHaveBeenCalledTimes(1);
    expect(canvases).toHaveBeenNthCalledWith(1, 40);
  });
});
