...
// @ffmpeg/core is already a direct dependency (see package.json) and its
// .js/.wasm are on disk in node_modules — there is no reason to fetch the
// same files over the network from unpkg.com on every export. That made
// every browser export (which always needs this fallback for audio
// extraction, since there's no native bridge outside the desktop app)
// depend on a third-party CDN being reachable: it fails outright offline,
// behind ad/tracker blockers that flag unpkg, on restrictive corporate or
// self-hosted networks, or simply if unpkg has an outage or CORS hiccup.
// `?url` makes Vite bundle these as local static assets instead, so the
// exact same files are served from this app's own origin.
