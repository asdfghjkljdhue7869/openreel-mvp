...
export class ExportFrameDecoder {
  ...
  // A forward gap bigger than this is cheaper to reseek (jump near the
  // nearest keyframe and decode from there) than to walk through
  // frame-by-frame in the loop below.
  private static readonly FORWARD_SEEK_RESET_THRESHOLD_SEC = 2;

  private async getSequentialFrame(timestamp: number): Promise<OffscreenCanvas | null> {
    if (!this.sink) return null;

    // Start a new sequential decode for the first request, whenever the
    // timeline moves backwards (reverse playback, a loop, or a later clip
    // reusing the same media from an earlier in-point), or whenever it jumps
    // far enough forward (trim in-point changes, scrubbing, seeking) that
    // decoding every intervening frame one at a time would be slower than
    // just reseeking. Without this, a large forward jump gets served by the
    // while loop below silently decoding-and-discarding every frame in
    // between inside a single getFrame() call, which shows up as a burst of
    // very slow "frames" and dropped-frame counts right after the jump.
    if (
      !this.canvasIterator ||
      (this.currentFrame &&
        (timestamp < this.currentFrame.timestamp - 1e-8 ||
          timestamp - this.currentFrame.timestamp >
            ExportFrameDecoder.FORWARD_SEEK_RESET_THRESHOLD_SEC))
    ) {
