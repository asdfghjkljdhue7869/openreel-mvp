...
export class PlaybackController {
  ...
  private handleClockTimeUpdate(time: number): void {
    ...
    if (this.masterClock.shouldSkipFrame()) {
      // Track that we're falling behind, but we must still render — driftMs
      // is only ever recalculated inside a successful render (via
      // reportVideoTime), so returning here without rendering means drift
      // can never shrink again: audio time keeps advancing while the "last
      // rendered" reference point stays frozen forever, permanently locking
      // shouldSkipFrame() to true. One slow frame would freeze playback for
  ...
  /**
   * Same as setProject(), but leaves playback state (playing/paused/stopped)
   * untouched. Use this for incremental edits to the project that already
   * being played — a timeline drag, a track toggle, adding a clip — where
   * forcing a stop on every edit would silently kill playback the instant
   * anything changes, without any visible indication of why. setProject()
   * itself is still correct for genuinely loading a new/different project.
   */
  updateProjectPreservingPlaybackState(project: Project): void {
    this.project = project;
    this.masterClock.setDuration(project.timeline.duration);
    this.pruneAudioBuffers(project);
  }
  ...
  private async renderFrameAtTime(time: number): Promise<void> {
    ...
    try {
      const { width: previewWidth, height: previewHeight } = this.getPreviewDimensions();
  ...
  /**
   * Computes the frame dimensions to request for realtime preview playback,
   * capped by config.previewMaxDimension. Returns undefined/undefined
   * (native resolution) when the source is already at or below the cap, or
   * when capping is disabled — VideoEngine.renderFrame treats undefined as
   * "use the project's native settings.width/height".
   */
  private getPreviewDimensions(): { width?: number; height?: number } {
    const cap = this.config.previewMaxDimension;
    if (!this.project || !cap) return { width: undefined, height: undefined };

    const { width, height } = this.project.settings;
    const longEdge = Math.max(width, height);
    if (longEdge <= cap) return { width: undefined, height: undefined };
