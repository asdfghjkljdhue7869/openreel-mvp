...
export class TitleEngine {
  ...
  // Rendering text via canvas fillText() and allocating a new full-frame
  // OffscreenCanvas is real, measurable per-frame cost (confirmed: ~7-8ms
  // added to frame render time while a title is active) — for static text
  // (no animation/keyframes/shader), the rasterized output is identical on
  // every frame within the clip's duration, so it's cached and reused
  // instead of re-rendered from scratch each time.
  private renderCache = new Map<string, { cacheKey: string; result: TextRenderResult }>();

  /**
   * Returns a cache key for static text content, or null if the clip has
   * any time-varying aspect (animation, keyframes, an emphasis animation, a
   * 3D pass, or a shader — shaders take `time` as an input and may animate
   * even without a formal TextAnimation). Caching a time-varying clip would
   * silently freeze its animation on whatever frame first got cached, so
   * those are deliberately never cached.
   */
  private computeStaticCacheKey(
    clip: TextClip,
    width: number,
    height: number,
  ): string | null {
  ...
  renderText(
    ...
    const cacheKey = this.computeStaticCacheKey(clip, width, height);
    if (cacheKey) {
      const cached = this.renderCache.get(clip.id);
    ...
    if (cacheKey) {
      this.renderCache.set(clip.id, { cacheKey, result });
    } else {
      // Clip is time-varying (or just became so) — make sure no stale
      // cached frame lingers for it.
