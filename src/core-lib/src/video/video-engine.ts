...
export class VideoEngine {
  ...
  async decodeFrameWithMediaBunny(
    ...
    try {
      ...
      if (mediaId) {
        // Reuse the same persistent, sequential decoder session export uses —
        // getFrameAtTime() below tears down and rebuilds the entire
        // demuxer/decoder from scratch on every single call, which is fine
        // for one-off thumbnails but means every frame during normal
        // sequential playback pays a fresh "seek to nearest keyframe, decode
        // forward" cost instead of just advancing one frame. This is
        // idempotent (get-or-create), so repeated calls after the first are
        // just a map lookup.
        const exportDecoder = await mediaEngine.createExportDecoder(
          mediaId,
          blob,
          width,
        );
  ...
  async decodeFrameWithVideoElement(
    ...
    if (!cached) {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        // 10s was long enough to freeze the whole page on a failed decode —
        // a local blob's video metadata should resolve in milliseconds when
        // it's going to resolve at all, so a much shorter timeout lets a
        // genuinely stuck decode recover fast instead of stalling playback.
  ...
  async decodeFrame(
    ...
    try {
      ...
      try {
        ...
        // If toVideoFrame() succeeded above but createImageBitmap() then
        // threw, the extracted VideoFrame is a separate object from
        // `sample` and would otherwise never get closed on this path.
        if (videoFrame && videoFrame !== sample) {
          (videoFrame as VideoFrame).close();
        }
