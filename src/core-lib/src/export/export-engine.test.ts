...
describe("ExportEngine", () => {
  ...
  describe("hardware encode retry", () => {
    function createRetryTestProject() {
      return createMockProject({
        timeline: createMockTimeline({
          tracks: [
            createMockTrack({
              clips: [createMockClip({ duration: 2, outPoint: 2 })],
            }),
          ],
          duration: 2,
        }),
    ...
    function createTruncatableWritableStream() {
      return {
        seek: vi.fn().mockResolvedValue(undefined),
        write: vi.fn().mockResolvedValue(undefined),
        close: vi.fn().mockResolvedValue(undefined),
    ...
    it("retries the whole export in software when hardware encoding throws mid-stream", async () => {
      let callCount = 0;
      mockVideoSourceAdd.mockImplementation(async () => {
        callCount++;
        if (callCount === 1) {
      ...
      const project = createRetryTestProject();
      const writableStream = createTruncatableWritableStream();
      ...
      // The failed attempt reset the output file before retrying...
      expect(writableStream.truncate).toHaveBeenCalledWith(0);
      expect(writableStream.seek).toHaveBeenCalledWith(0);
      // ...a fresh backend/encoder was constructed for the retry...
      expect(mockVideoSourceConfigs.length).toBe(2);
      // ...and the export ultimately succeeded via the software retry.
      expect(result?.success).toBe(true);
    });

    it("does not retry a second time if software encoding also fails", async () => {
      mockVideoSourceAdd.mockImplementation(async () => {
        throw new DOMException("Encoding error.", "EncodingError");
      });
      ...
      // Exactly one retry: original attempt + one software retry, not an infinite loop.
      expect(mockVideoSourceConfigs.length).toBe(2);
      expect(result?.success).toBe(false);
    });

    it("does not retry non-encoder errors (e.g. user cancellation)", async () => {
      mockRenderFrame.mockImplementationOnce(async () => {
        throw new Error("Some unrelated rendering failure");
      });
      ...
      // Only one attempt — non-encoder errors must not trigger the retry path.
      expect(mockVideoSourceConfigs.length).toBe(1);
      expect(writableStream.truncate).not.toHaveBeenCalled();
      expect(result?.success).toBe(false);
    });
  });
});
