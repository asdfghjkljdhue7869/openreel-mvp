...
export interface VideoExportSettings {
  ...
  /**
   * Skip the hardware-encoder attempt entirely and start directly in
   * software. On a machine with a known-flaky GPU driver, the default
   * hardware-first-then-retry-in-software behavior means every single
   * export wastes time on a doomed hardware attempt and hits the same
   * crash-prone driver path before falling back. If hardware encoding
   * has already failed here before, there's nothing to gain by
   * attempting it again — set this once and exports go straight to the
   * software path that's already proven reliable.
   */
  preferSoftwareEncoding?: boolean;
}
