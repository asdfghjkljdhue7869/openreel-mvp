...
export class ExportEngine {
  ...
  async *exportVideo(
    ...
    let hardwareEncodeRetryAttempted = false;
    let forceSoftwareEncoding = false;

    while (true) {
    const backend = encoderBackendFactory
      ? encoderBackendFactory(this.mediabunny)
      : new WebCodecsBackend(this.mediabunny!, {
          hardwareAcceleration: forceSoftwareEncoding ? "no-preference" : "prefer-hardware",
        });
    ...
    if (backend.requiresWebCodecsClamping) {
      // The old clamp always capped at a hardcoded 3840x2160 (or 1920x1080
      // for heavier codecs), even on machines that could genuinely encode
      // higher. Ask the browser's real WebCodecs encoder whether it can
      // actually handle the requested resolution/codec combination before
      // falling back to the conservative heuristic. This is what makes
      // >4K exports possible at all when the hardware supports it, while
      // still protecting weaker machines/browsers that can't.
      const canEncodeAtRequestedSize = await this.canEncodeResolution(
        fullSettings.codec,
        fullSettings.width,
        fullSettings.height,
      ...
      console.info(
        `[export] resolution capability probe: ${fullSettings.width}x${fullSettings.height} -> ${canEncodeAtRequestedSize ? "encodable, no clamp" : "not encodable, falling back to heuristic clamp"}`,
      );

      if (!canEncodeAtRequestedSize) {
    ...
    try {
      ...
      // Our own ExportError objects are plain objects (never Error/DOMException
      // instances), but DOMException also happens to expose a legacy `.code`
      // getter on its prototype, so a naive `"code" in error` check misfires on
      // real encoder errors. Explicitly rule out real exception types first.
      const isRealException = error instanceof Error || error instanceof DOMException;
      const isStructuredError =
        !isRealException && !!error && typeof error === "object" && "code" in error;
      const looksLikeEncoderFailure =
        isRealException &&
        (error instanceof DOMException
          ? error.name === "EncodingError" || error.name === "OperationError"
          : /encod/i.test((error as Error).message));

      if (
        looksLikeEncoderFailure &&
        !forceSoftwareEncoding &&
        !hardwareEncodeRetryAttempted &&
        !encoderBackendFactory &&
        writableStream
      ) {
        hardwareEncodeRetryAttempted = true;
        try {
          ...
          return {
            ...
            error: this.createError(
              "FRAME_ENCODE_FAILED",
              `Hardware video encoding failed (${error instanceof Error ? error.message : "unknown encoder error"}), and the output file could not be reset to retry with software encoding: ${resetError instanceof Error ? resetError.message : "unknown error"}.`,
              "rendering",
            ),
          };
        }
        console.warn(
          "[export] Hardware video encoding failed mid-export, retrying the full export with software encoding:",
          error,
        );
        // A decode/encode failure like this leaves the underlying WebCodecs
        // decoder/encoder instances in a permanently closed, unusable state
        // (per spec — once a VideoDecoder errors, it stays errored). Without
        // this cleanup, the software retry above would keep calling into
        // those same dead decoders on every single frame, which either
        // silently fails or never resolves at all — looking exactly like a
        // hang with zero further progress logged, rather than an error.
        try {
    ...
    } // end while(true) hardware-encode retry loop
  }
  ...
  // Probes the browser's real WebCodecs encoder to see if it can actually
  // handle this resolution/codec/framerate, reusing the exact same
  // capability check mediabunny uses internally for codec selection
  // (getFirstEncodableVideoCodec) rather than guessing from a hardcoded
  // constant. Returns false (triggering the conservative fallback clamp)
  // on any probe failure — an unclampable environment should never bypass
  // the safety net, only a confirmed-capable one should.
  private async canEncodeResolution(
    codec: VideoExportSettings["codec"],
    width: number,
    height: number,
    frameRate: number,
  ): Promise<boolean> {
    if (!this.mediabunny) return false;
    try {
      const { getFirstEncodableVideoCodec } = this.mediabunny;
      const mediabunnyCodec = CODEC_MAP[codec as keyof typeof CODEC_MAP];
      if (!mediabunnyCodec) return false;

      const result = await getFirstEncodableVideoCodec(
        [mediabunnyCodec] as Parameters<typeof getFirstEncodableVideoCodec>[0],
        {
          width,
          height,
          bitrate: 8_000_000,
