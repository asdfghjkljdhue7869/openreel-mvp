...
export class WebCodecsBackend implements EncoderBackend {
  ...
  async addVideoFrame(
    ...
    try {
      ...
      // If videoSource.add() throws (e.g. a hardware encoder failure mid
      // export, which does happen on some GPU/driver combos and is exactly
      // what triggers export-engine.ts's software-encoding retry), both the
      // VideoSample and the ImageBitmap it wraps would otherwise be
      // abandoned with no close() call — the exact "VideoSample was garbage
      // collected without first being closed" warning.
      videoSample.close();
      frame.close();
    }
  }
