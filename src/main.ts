...
const timelineUI = new TimelineUI(timelineRulerEl, timelineTracksEl, timelineTimecodeEl, {
  onSeek: (t) => {
    playback?.seek(t);
  },
  onProjectChanged: (updatedProject) => {
    ...
    log(`Timeline updated: ${updatedProject.timeline.tracks.length} track(s).`);
  },
...
async function importClipToTrack(trackId: string) {
  ...
  input.addEventListener("change", async () => {
    ...
    try {
      ...
      const newClip = createClip(mediaItem.id, trackId, lastEnd, clipDuration);
