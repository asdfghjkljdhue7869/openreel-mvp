...
- `openreel-mvp/` — the actual Vite + vanilla TS app (`src/main.ts`,
  `src/index.html`, `src/theme-default.css`).
- `openreel-mvp/src/core-lib/` — a full copy of `packages/core` from
  `Augani/openreel-video`, with the fixes above already applied. This is
  what `@openreel/core` resolves to via Vite aliases.
- Patches generated against a clean clone of `Augani/openreel-video` for
  each fix are meant to be applied there directly if contributing
  upstream, separate from the MVP app itself.
