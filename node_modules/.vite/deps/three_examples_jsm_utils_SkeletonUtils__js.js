import { Ao as SkeletonHelper, Is as VectorKeyframeTrack, Ps as Vector3, Xr as Matrix4, da as Quaternion, fa as QuaternionKeyframeTrack, x as AnimationMixer, y as AnimationClip } from "./three.module-BQ0TNVQb.js";
...
function getBoneName(bone, options) {
	if (options.getBoneName !== void 0) return options.getBoneName(bone);
...
function retarget(target, source, options = {}) {
		...
		if (boneTo) {
			...
			if (target.isObject3D) {
				if (options.localOffsets) {
					if (options.localOffsets[bone.name]) globalMatrix.multiply(options.localOffsets[bone.name]);
				}
			}
	...
	if (options.preserveBonePositions) for (let i = 0; i < bones.length; ++i) {
		bone = bones[i];
...
function retargetClip(target, source, clip, options = {}) {
	...
	for (let frame = 0; frame < end; ++frame) {
		...
		if (frame === end - 2) mixer.update(delta - 1e-7);
	...
	for (let i = 0; i < boneDatas.length; ++i) {
		...
		if (boneData) {
			if (boneData.pos) convertedTracks.push(new VectorKeyframeTrack(".bones[" + boneData.bone.name + "].position", boneData.pos.times, boneData.pos.values));
...
function getBoneByName(name, skeleton) {
	for (let i = 0, bones = getBones(skeleton); i < bones.length; i++) if (name === bones[i].name) return bones[i];
}
